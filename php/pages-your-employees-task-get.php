<?php 
	...
	class pagesYourEmployeesTaskGet extends dB
	{
		public $userId 			= null;
		...
		/*
			...
			phone
		*/
		public function getYourEmployeesTaskData($ownerUserId){
			/*
			...
			*/
			$yourTask = $this->db->innerJoin("task.*, project.*, employee.*,user.userName, user.userSurname",
											 "task",
											  ["project"=>"project.projectId = task.projectId",
											   "user"	=>"user.userId = project.creatingProject",
											   "employee"=>"employee.employeeId = task.dutyEmployeeId"],
											  "WHERE employee.ownerUser = {$ownerUserId}" 
											);
			//var_dump($yourTask);											  
			...
			foreach($yourTask as $key => $value){
				...
				$li=$li."
					...
					</label><br></div><div class='item-date'><div class='row'><div class='col-6'>
	...
	//$yourEmployeesTaskData = new pagesYourEmployeesTaskGet;
?>